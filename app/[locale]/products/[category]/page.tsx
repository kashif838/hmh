...
export async function generateMetadata({ params }: PageProps<"/[locale]/products/[category]">) {
  ...
  return pageMeta({ title: tr(c.name), description: `${tr(c.intro)} ${tr("Traded by HMH General Trading LLC from Dubai.")}`, path: `/products/${c.slug}`, locale, image: c.image?.src });
}
...
export default async function CategoryPage({ params }: PageProps<"/[locale]/products/[category]">) {
  ...
  return (
    <Shell>
      <PageIntro
        crumbs={[{ name: tr("Products"), path: "/products" }, { name: tr(c.name), path: `/products/${c.slug}` }]}
        title={tr(c.name)}
        lead={tr(c.intro)}
        image={c.image && { ...c.image, alt: tr(c.image.alt) }}
      >
        <a href="#rfq" className="btn btn-dark mt-7">{tr("Request a Quote")}</a>
      </PageIntro>

      <section className="bg-white pb-20 lg:pb-28">
        <div className="wrap grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-6">
          <h2 data-reveal className="t-h2-sm lg:col-span-4">{tr("What we trade")}</h2>
          <ul data-reveal className="m-0 list-none border-b border-ink-3/11 p-0 lg:col-span-8">
            {c.items.map((it) => (
              <li key={it} className="flex items-center justify-between gap-6 border-t border-ink-3/11 py-[18px]">
                <span className="text-[17px] font-medium tracking-[-0.015em]">{tr(it)}</span>
                <span className="text-[13px] font-light text-muted">{tr("Quoted per order")}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <section className="bg-off-white py-20 lg:py-[100px]">
          <div className="wrap">
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {own.map((p) => (
                <ProductCard key={p.slug} href={`/products/${c.slug}/${p.slug}`} image={p.cutout} contain tag={tr(p.brand === "boon" ? "Boon" : "Abu Koora")} name={tr(p.name)} meta={tr(p.facts[0][1])} />
              ))}
            </div>
          </div>
        </section>
        ...
        <div className="wrap grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-6">
          <div data-reveal className="lg:col-span-5 lg:pe-10">
            ...
            <p className="mt-5 text-[14.5px] font-light leading-[1.74] text-body">
              {tr("Send the product, the volume, the destination and the Incoterm you trade on. Pricing, minimum order and lead time are confirmed on every quotation.")}
            </p>
            <h3 className="t-label mt-12 text-gold-deep">{tr(c.group === "food" ? "Other food categories" : "Other non-food categories")}</h3>
            <ul className="mt-4 flex list-none flex-wrap gap-2 p-0">
              {siblings.map((s) => (
                <li key={s.slug}><Link href={`/products/${s.slug}`} className="inline-flex border border-ink-3/20 px-[13px] py-[7px] text-[12.5px] font-medium hover:border-ink-3">{tr(s.name)}</Link></li>
              ))}
            </ul>
          </div>
