...
export async function generateMetadata({ params }: PageProps<"/[locale]">) {
  ...
  return {
    ...pageMeta({ title: tr("From origin to every market"), description: tr(company.description), path: "/", locale }),
    title: { absolute: tr("HMH General Trading — From origin to every market") },
  };
}
...
export default async function Home({ params }: PageProps<"/[locale]">) {
  ...
  return (
    <Shell overlay>
      <HeroCarousel slides={SLIDES.map((s) => ({ ...s, alt: tr(s.alt), label: tr(s.label) }))}>
        <div className="max-w-[830px]">
          <h1 data-rise className="t-display text-paper-bright">{tr("From origin to")} <Hl tone="gold">{tr("every market")}</Hl></h1>
          ...
          <div data-rise className="mt-8 flex flex-col gap-3 sm:flex-row sm:gap-[14px] lg:mt-[42px]" style={{ transitionDelay: ".24s" }}>
            <Link href="/products" className="btn btn-gold">{tr("Explore What We Trade")} <Arrow /></Link>
        ...
        <div className="wrap">
          <div className="grid grid-cols-1 items-end gap-10 lg:grid-cols-12 lg:gap-6">
            <h2 data-reveal className="t-h2 lg:col-span-7 lg:text-[54px]">
              {tr("A Dubai trading house built on quality products and")} <Hl>{tr("reliable service")}</Hl>
            </h2>
          ...
          <div data-reveal className="tile relative mt-[52px] h-[260px] bg-ink md:h-[340px] lg:h-[398px]">
            <Image src="/images/about-wide.jpg" alt={tr("Container vessel seen from directly above on open water")} fill sizes="100vw" className="object-cover" />
          </div>
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-12">
            <p data-reveal className="m-0 text-[12.5px] font-medium leading-[1.6] text-gold-deep md:col-span-3">
              {tr("Cross-trading: shipments routed origin to destination, whether or not they transit the UAE.")}
            </p>
            <p data-reveal className="m-0 text-[15px] font-light leading-[1.76] text-body md:col-span-4" style={{ transitionDelay: ".06s" }}>
              {tr("One of the pioneers in trading for food and non-food products, HMH serves customers and brand owners across the GCC, MENA, CIS and African markets, with a professional team, competitive pricing and a portfolio spanning FMCG and branded goods.")}
            </p>
            <p data-reveal className="m-0 text-[15px] font-light leading-[1.76] text-body md:col-span-4" style={{ transitionDelay: ".12s" }}>
              {tr("As trade has shifted, so have we. Import and export, cross-trading, wholesale distribution and private labelling now sit alongside one another: one company, one point of accountability, from sourcing through to shelf.")}
            </p>
          </div>
        </div>
      ...
      <section id="products" className="bg-white pb-20 pt-4 lg:pb-32 lg:pt-6">
        <div className="wrap">
          <SectionHead
            title={<>{tr("Thirteen categories, food and")} <Hl>{tr("non-food")}</Hl></>}
            aside={
              <>
                <p className="m-0">{tr("Traded in container loads against specification, origin and Incoterm. Prices are quoted on enquiry, never listed.")}</p>
      ...
      <section id="brands" className="bg-white pt-20 lg:pt-[122px]">
        <div className="wrap mb-14">
          <SectionHead
            title={<>{tr("We don’t only move brands. We")} <Hl>{tr("build them")}</Hl></>}
            aside={tr("Brands owned and developed by HMH: specified, packaged bilingually and placed in market.")}
          />
