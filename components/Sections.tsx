...
export function SectionHead({ title, aside, dark = false }: { title: React.ReactNode; aside?: React.ReactNode; dark?: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between lg:gap-[70px]">
      <h2 data-reveal className={`t-h2 max-w-[680px] ${dark ? "text-paper-bright" : ""}`}>{typeof title === "string" ? tr(title) : title}</h2>
      {aside && (
        <div data-reveal className={`max-w-[360px] text-[14px] font-light leading-[1.72] lg:mb-2 ${dark ? "text-paper/70" : "text-muted"}`} style={{ transitionDelay: ".08s" }}>
          {typeof aside === "string" ? tr(aside) : aside}
        </div>
      )}
    </div>
  );
  ...
  return (
    <figure data-reveal className="m-0">
      ...
      <blockquote className="m-0 mt-[10px]">
        <p className={`m-0 font-normal leading-[1.58] tracking-[-0.017em] text-[#26272a] ${size === "lg" ? "text-[17px] lg:text-[19px]" : "text-[16px] lg:text-[17px]"}`}>{tr(text)}</p>
      </blockquote>
      <figcaption className="t-label mt-[14px] text-gold-deep">{tr(caption)}</figcaption>
    </figure>
  );
}
    ...
    <div className="grid grid-cols-1 gap-px border border-ink-3/13 bg-ink-3/13 sm:grid-cols-2 lg:grid-cols-3">
      {capabilities.map((c, i) => (
        <Link key={c.title} href={c.href} data-reveal className="cap block bg-white px-7 pb-9 pt-[34px] lg:px-[30px]" style={{ transitionDelay: `${(i % 3) * 0.05}s` }}>
          ...
          <h3 className="mt-[18px] mb-[9px] text-[18px] tracking-[-0.02em]">{tr(c.title)}</h3>
          <p className="m-0 text-[13px] font-light leading-[1.66] text-muted">{tr(c.body)}</p>
        </Link>
      ))}
    </div>
  );
...
function CategoryTile({ c, index }: { c: (typeof categories)[number]; index: number }) {
    ...
    return (
      <Link href={`/products/${c.slug}`} data-reveal className={`tile relative block bg-off-white ${h} sm:col-span-1 lg:[grid-column:span_var(--span)]`} style={{ ["--span" as string]: c.span, transitionDelay: `${(index % 4) * 0.05}s` }}>
        ...
        <span className="absolute inset-x-6 bottom-[22px] block">
          ...
          <span className="block text-[21px] font-semibold tracking-[-0.024em] text-[#1c1d1f]">{tr(c.name)}</span>
          <span className="mt-[5px] block text-[12.5px] font-light text-muted">{c.items.map(tr).join(" · ")}</span>
        </span>
      </Link>
    );
  }
  return (
    <Link href={`/products/${c.slug}`} data-reveal className={`tile relative block bg-[#111315] ${h} lg:[grid-column:span_var(--span)]`} style={{ ["--span" as string]: c.span, transitionDelay: `${(index % 4) * 0.05}s` }}>
      <Image src={c.image.src} alt={tr(c.image.alt)} fill sizes={big ? "(min-width:1024px) 50vw, 100vw" : "(min-width:1024px) 33vw, (min-width:640px) 50vw, 100vw"} className="object-cover saturate-[.92]" />
      ...
      <span className="absolute inset-x-[22px] bottom-6 block lg:inset-x-[26px]">
        ...
        <span className="flex items-end justify-between gap-[18px]">
          <span className="block">
            <span className={`block font-semibold tracking-[-0.026em] text-paper-bright ${big ? "text-[22px] lg:text-[27px]" : "text-[20px] lg:text-[21px]"}`}>{tr(c.name)}</span>
            <span className="mt-[6px] block text-[12.5px] font-light text-paper/75">{c.items.map(tr).join(" · ")}</span>
          </span>
  ...
  return (
    <>
      <div data-reveal className="mb-[18px] mt-10 flex items-center gap-4">
        <span className="t-label text-gold-deep">{tr("Food")}</span>
        <span className="h-px flex-1 bg-ink-3/18" />
        <span className="text-[12.5px] text-[#7a756b]">{food.length} {tr("categories")}</span>
      </div>
      ...
      <div id="non-food" data-reveal className="mb-[18px] mt-11 flex scroll-mt-24 items-center gap-4">
        <span className="t-label text-gold-deep">{tr("Non-food")}</span>
        <span className="h-px flex-1 bg-ink-3/18" />
        <span className="text-[12.5px] text-[#7a756b]">{nonFood.length} {tr("categories")}</span>
      </div>
      <div className="grid grid-cols-1 gap-[10px] sm:grid-cols-2 lg:grid-cols-12">
...
export function WorldMap() {
  ...
  return (
    <svg viewBox="150 26 1006 496" width="100%" height="auto" direction="ltr" role="img" className="block"
      aria-label={tr("World map with HMH's trading regions highlighted around the Gulf, MENA, CIS, Africa and Australia, and routes radiating from Dubai")}>
      ...
      <g>
        {ROUTES.map((r) => (
          <g key={r.name}>
            ...
            <text x={r.x + r.lx} y={r.y + r.ly} textAnchor={r.anchor} fontFamily="var(--font-poppins), sans-serif" fontSize="9.5" fontWeight="600" letterSpacing={ar ? 0 : 1.8} fill="rgba(234,230,220,.82)">{tr(r.name)}</text>
          </g>
        ))}
      </g>
      ...
      <text x={DXB.x} y="210" textAnchor="middle" fontFamily="var(--font-poppins), sans-serif" fontSize="11" fontWeight="600" letterSpacing={ar ? 0 : 2.2} fill="#f4f1ea">{tr("DUBAI")}</text>
    </svg>
  );
}
    ...
    <section id="markets" className="bg-ink pt-20 lg:pt-[122px]">
      <div className="wrap">
        <div className="mx-auto max-w-[880px] text-center">
          <Title data-reveal className="t-h2 text-paper-bright">{tr("Dubai sits within eight hours of")} <Hl tone="gold">{tr("two-thirds of the world")}</Hl></Title>
          <p data-reveal className="mx-auto mt-[22px] max-w-[600px] text-[15px] font-light leading-[1.72] text-paper/70" style={{ transitionDelay: ".08s" }}>
            {tr("That geography is the reason the business exists here. Goods route through the UAE, or around it, depending on what the trade needs.")}
          </p>
        </div>
        ...
        <div data-reveal className="mt-[22px] flex flex-wrap items-center justify-center gap-x-7 gap-y-3 text-[12.5px] font-light text-paper/55">
          <span className="flex items-center gap-2"><span className="h-[9px] w-[9px] rounded-full bg-gold opacity-75" />{tr("Active trading regions")}</span>
          <span className="flex items-center gap-2"><span className="h-px w-5 bg-gold" />{tr("Market routes")}</span>
          <span className="flex items-center gap-2"><span className="h-px w-5 bg-paper/60" />{tr("Sourcing origin")}</span>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4">
          {markets.map((m, i) => (
            <div key={m.name} data-reveal className={`py-7 pe-5 ${i > 0 ? "lg:border-s lg:border-paper/12 lg:ps-[22px]" : ""} ${i % 2 === 1 ? "border-s border-paper/12 ps-5 lg:ps-[22px]" : ""}`} style={{ transitionDelay: `${i * 0.05}s` }}>
              <h3 className="mb-[9px] text-[17px] tracking-[-0.02em] text-[#f4f1ea]">{tr(m.name)}</h3>
              <p className="m-0 text-[13px] font-light leading-[1.64] text-paper/62">{tr(m.body)}</p>
            </div>
          ))}
        </div>
        ...
        <p data-reveal className="mx-auto mt-5 max-w-[660px] text-center text-[12.5px] font-light leading-[1.7] text-paper/50">
          {tr("HMH also trades into the Australian continent.")}
        </p>
      </div>
      {withBand ? (
        <div data-reveal className="relative mt-[74px] h-[240px] overflow-hidden lg:h-[264px]">
          <Image src="/images/dubai.jpg" alt={tr("Dubai waterfront skyline at dusk")} fill sizes="100vw" className="object-cover object-[50%_62%]" />
          ...
          <span className="absolute inset-0 flex items-center">
            <span className="wrap block w-full">
              <span className="block text-[21px] font-semibold tracking-[-0.024em] text-paper-bright lg:text-[24px]">{tr("Head office:")} {tr(`${company.city}, ${company.country}`)}</span>
              <span className="mt-2 block max-w-[440px] text-[13.5px] font-light leading-[1.66] text-paper/75">{tr("Trading, sourcing and private-label operations all run from here.")}</span>
            </span>
          </span>
        </div>
    ...
    <div data-reveal className="bg-ink-2 py-16 text-paper lg:py-[74px]">
      <div className="wrap flex flex-col gap-12 lg:flex-row lg:gap-[68px]">
        <div className="lg:w-[322px] lg:shrink-0">
          ...
          <p className="mt-3 text-[18px] tracking-[-0.02em] text-gold">{tr(boon.tagline)}</p>
          <p className="t-label mt-4 text-paper/55">{tr("HMH own brand")}</p>
          <p className="mt-6 text-[13.5px] font-light leading-[1.74] text-paper/75">{tr(boon.summary)}</p>
          <dl className="mt-7 grid grid-cols-[auto_1fr] gap-x-5 gap-y-[11px] text-[12.5px] font-light">
            {boon.facts.map(([k, v]) => (<Fragment key={k}><dt className="text-paper/50">{tr(k)}</dt><dd className="m-0 text-paper">{tr(v)}</dd></Fragment>))}
          </dl>
          <div className="mt-[30px] flex flex-wrap gap-3">
            <Link href="/partner-with-us" className="btn btn-gold">{tr("Become a Distributor")}</Link>
            <Link href="/brands/boon" className="btn btn-ghost">{tr("The range")}</Link>
          </div>
        </div>
        <div className="min-w-0 flex-1">
          <div className="cupwrap -mx-5 flex items-end gap-4 overflow-x-auto px-5 pb-2 lg:mx-0 lg:items-start lg:justify-between lg:gap-2 lg:overflow-visible lg:px-[14px]">
            {withCups.map((p, i) => (
              <Link key={p.slug} href={`/products/${p.category}/${p.slug}`} className="flex shrink-0 flex-col items-center gap-[14px] lg:min-w-0 lg:flex-1 lg:shrink">
                <Image className="cup h-[150px] w-auto lg:h-auto lg:max-h-[198px] lg:w-full lg:object-contain" style={{ transitionDelay: `${i * 0.04}s` }} src={p.cutout!} alt={tr(p.name)} width={280} height={356} sizes="(min-width:1024px) 130px, 118px" />
                <span className="text-center text-[12.5px] font-light text-paper/75">{tr(p.shortName)}</span>
              </Link>
            ))}
          </div>
          ...
          <p className="mt-[18px] flex flex-wrap items-center gap-[10px] text-[12.5px] font-light text-paper/60">
            <span className="t-label text-paper/45">{tr("Also in range")}</span>
            {others.map((p, i) => (
              <span key={p.slug} className="flex items-center gap-[10px]">
                {i > 0 && <span className="opacity-40">&middot;</span>}
                <Link href={`/products/${p.category}/${p.slug}`} className="hover:text-gold-light">{tr(p.shortName)}</Link>
              </span>
            ))}
          </p>
        </div>
        ...
        <div className="grid w-full flex-1 grid-cols-2 items-end gap-4 lg:gap-[26px]">
          {[["/images/abukoora-en.png", "English pack"], ["/images/abukoora-ar.png", "Arabic pack"]].map(([src, label]) => (
            <figure key={src} className="m-0 flex flex-col gap-[14px]">
              <Image src={src} alt={tr(`Abu Koora sandwich biscuits carton, ${label.toLowerCase()}`)} width={808} height={860} sizes="(min-width:1024px) 30vw, 50vw" className="h-auto w-full" />
              <figcaption className="t-label text-center text-[#7a6a4a]">{tr(label)}</figcaption>
            </figure>
          ))}
        </div>
        <div className="lg:w-[400px] lg:shrink-0">
          <h3 className="text-[42px] font-bold leading-none tracking-[-0.03em] text-[#1c1d1f] lg:text-[50px]">{tr(ak.name)}</h3>
          <p className="mt-3 text-[18px] tracking-[-0.02em] text-gold-deep">{tr(ak.tagline)}</p>
          <p className="t-label mt-4 text-muted">{tr("HMH own brand")} &middot; {tr("Private label")}</p>
          <p className="mt-6 text-[13.5px] font-light leading-[1.74] text-body">{tr(ak.summary)}</p>
          {ak.facts.length > 0 && <dl className="mt-7 grid grid-cols-[auto_1fr] gap-x-5 gap-y-[11px] text-[12.5px] font-light">
            {ak.facts.map(([k, v]) => (<Fragment key={k}><dt className="text-[#7a756b]">{tr(k)}</dt><dd className="m-0 text-[#2a2b2d]">{tr(v)}</dd></Fragment>))}
          </dl>}
          <div className="mt-[30px] flex flex-wrap gap-3">
            <Link href="/products/biscuits-confectionery/abu-koora-sandwich-biscuits" className="btn btn-dark">{tr("View product")}</Link>
            <Link href="/services/private-label" className="btn btn-line">{tr("How we build a brand")}</Link>
          </div>
        </div>
      </div>
    </div>
        ...
        {privateLabelSteps.map((s, i) => {
          ...
          return (
            <li key={s.title} data-reveal className="lg:pe-7" style={{ transitionDelay: `${i * 0.06}s` }}>
              ...
              <span className={`t-label mt-[26px] block ${last ? "text-gold-light" : "text-gold"}`}>{tr("Step")} 0{i + 1}</span>
              <h3 className="mt-3 mb-[10px] text-[25px] tracking-[-0.028em] text-paper-bright">{tr(s.title)}</h3>
              <p className="m-0 text-[13px] font-light leading-[1.7] text-paper/66">{tr(s.body)}</p>
            </li>
          );
        })}
      </ol>
  ...
  return (
    <section id="private-label" className="bg-ink pb-20 lg:pb-[126px]">
      <div className="relative h-[340px] overflow-hidden lg:h-[376px]">
        <Image src="/images/pl-crane.jpg" alt={tr("Ship-to-shore cranes above stacked containers")} fill sizes="100vw" className="object-cover" />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,11,12,.52),rgba(10,11,12,.97))]" />
        <div className="absolute inset-0 flex items-center">
          <div className="wrap w-full">
            <h2 data-reveal className="t-h2 max-w-[800px] text-paper-bright">{tr("Bring us a shelf. We’ll bring you")} <Hl tone="gold">{tr("a product to put on it")}</Hl></h2>
          </div>
        </div>
      </div>
      <div className="wrap relative -mt-[18px]">
        <p data-reveal className="mb-[54px] max-w-[616px] text-[15px] font-light leading-[1.74] text-paper/72">
          {tr("Retailers and distributors come to us for product under their own name. We handle the whole route, and we have walked it ourselves, twice, with Abu Koora and Boon.")}
        </p>
      </div>
      <PrivateLabelSteps />
      <div className="wrap mt-12">
        <Link href="/services/private-label" className="btn btn-ghost">{tr("Private label in detail")} <Arrow /></Link>
      </div>
    </section>
  );
}
    ...
    <section className="section-y bg-white">
      <div className="wrap flex flex-col gap-12 lg:flex-row lg:gap-[90px]">
        <div className="lg:w-[428px] lg:shrink-0">
          <h2 data-reveal className="t-h2-sm">{tr("No inflated numbers. Just what we actually")} <Hl>{tr("do")}</Hl></h2>
          <p data-reveal className="mt-[26px] text-[14.5px] font-light leading-[1.74] text-body" style={{ transitionDelay: ".08s" }}>
            {tr("Plenty of trading companies lead with counters: countries, containers, customers. We would rather describe the work and let the enquiry test it.")}
          </p>
        ...
        <div className="min-w-0 flex-1 border-y border-ink-3/17">
          {reasons.map((r, i) => (
            <div key={r.title} data-reveal className={`py-7 ${i > 0 ? "border-t border-ink-3/12" : ""}`} style={{ transitionDelay: `${i * 0.05}s` }}>
              <h3 className="mb-2 text-[19px] tracking-[-0.026em] text-[#1c1d1f] lg:text-[21px]">{tr(r.title)}</h3>
              <p className="m-0 max-w-[560px] text-[14px] font-light leading-[1.72] text-muted">{tr(r.body)}</p>
            </div>
          ))}
        </div>
      </div>
...
export function ContactBand() {
  ...
  return (
    <section id="contact" className="relative overflow-hidden bg-ink py-20 lg:flex lg:h-[512px] lg:items-center lg:py-0">
      <Image src="/images/cta-yard.jpg" alt={tr("Container yard with gantry cranes")} fill sizes="100vw" className="object-cover" />
      ...
      <div className="wrap relative flex w-full flex-col gap-12 lg:flex-row lg:items-center lg:justify-between lg:gap-[70px]">
        <div className="max-w-[700px]">
          <h2 data-reveal className="t-h2 text-paper-bright">{tr("Tell us what you need")}<br className="hidden sm:block" /> {tr("and")} <Hl tone="gold">{tr("where it has to land")}</Hl></h2>
          <p data-reveal className="mt-6 max-w-[530px] text-[15px] font-light leading-[1.72] text-paper/80" style={{ transitionDelay: ".08s" }}>
            {tr("Send the category, the volume, the destination port and the Incoterm you trade on. We will come back with a quotation, not a brochure.")}
          </p>
          <div data-reveal className="mt-9 flex flex-wrap gap-[14px]" style={{ transitionDelay: ".16s" }}>
            <Link href="/request-a-quote" className="btn btn-gold">{tr("Request a Quote")}</Link>
            <Link href="/partner-with-us" className="btn btn-ghost">{tr("Partner With HMH")}</Link>
          </div>
        </div>
        <div data-reveal className="shrink-0 border border-paper/20 bg-ink/50 px-[34px] py-[30px]" style={{ transitionDelay: ".2s" }}>
          <p className="t-label m-0 text-gold">{tr("Direct")}</p>
          ...
          <p className="m-0 text-[12.5px] font-light leading-[1.68] text-paper/65">{tr(company.legalName)}<br />{tr(`${company.city}, ${company.country}`)}</p>
        </div>
      </div>
    </section>
  );
