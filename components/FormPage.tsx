...
export function FormPage({
  ...
  return (
    <>
      <Breadcrumbs items={[{ name: tr(name), path }]} />
      <section className="bg-white pb-24 pt-10 lg:pb-32 lg:pt-14">
        <div className="wrap grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-6">
          <div className="lg:col-span-5 lg:pe-10">
            <h1 data-reveal className="t-h2">{tr(title)} <Hl>{tr(highlight)}</Hl></h1>
            <p data-reveal className="t-lead mt-6 text-body">{tr(lead)}</p>
            {aside}
            <div data-reveal className="mt-10 border-t border-ink-3/11 pt-8">
              <p className="t-label m-0 text-gold-deep">{tr("Direct")}</p>
              ...
              <p className="mt-4 text-[13px] font-light leading-[1.6] text-muted">{tr(company.legalName)}<br />{tr(`${company.city}, ${company.country}`)}</p>
            </div>
          </div>
