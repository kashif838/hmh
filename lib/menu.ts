...
/** Plain data for the Products dropdown, built (and translated) on the server so the header ships no content code. */
export function getMenu(locale: string): Menu {
  const tr = translate(locale);
  const cat = (group: "food" | "non-food") =>
    categories.filter((c) => c.group === group).map((c) => ({ href: `/products/${c.slug}`, name: tr(c.name) }));
  const prod = (brand: "boon" | "abu-koora") =>
    productsOf(brand).map((p) => ({ href: `/products/${p.category}/${p.slug}`, name: tr(p.shortName) }));
